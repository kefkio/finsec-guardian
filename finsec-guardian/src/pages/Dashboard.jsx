...
function StatCard({ label, value, change, icon: Icon, trend, bgGradient, onClick }) {
  const isPositive = change?.startsWith("+") || trend === "up";
  const TrendIcon = trend === "up" ? TrendingUp : TrendingDown;
  
  return (
    <Card
      className="relative overflow-hidden border-border/50 bg-card/50 backdrop-blur-xl hover:bg-card/70 transition-all duration-300 group cursor-pointer"
      onClick={onClick}
    >
      ...
      <div className="relative z-10 space-y-8">
        ...
        <Card className="border-border/50 bg-card/50 backdrop-blur-xl">
          ...
          <CardContent className="p-6">
            <div className="space-y-3">
              {recentScans.map((scan, i) => {
                ...
                return (
                  <div
                    key={i}
                    onClick={() => scan.id && navigate(`/scanner/${scan.id}`)}
                    className={`group relative rounded-lg border transition-all duration-300 hover:border-border hover:bg-muted/50 ${scan.id ? 'cursor-pointer' : ''} ${severityBg[scan.severity]} ${severityColor[scan.severity].split(" ").pop()}`}
                  >
            ...
            <button
              onClick={() => navigate("/scanner")}
              className="w-full mt-4 px-4 py-3 rounded-lg border border-border/50 bg-muted/50 text-muted-foreground hover:bg-muted transition-colors text-sm font-mono uppercase tracking-wide cursor-pointer"
            >
              View All Scans →
            </button>
          </CardContent>
